...
static inline void fd_check(int fa);
...
int fs_close(int fd);
void init_fs() {
  // TODO: initialize the size of /dev/
...
size_t fs_read(int fd, void *buf, size_t len)
{
  ...
  {
    ret = file_table[fd].read(buf, file_table[fd].open_offset + file_table[fd].disk_offset, read_len);
  }
  else
  {
    ret = ramdisk_read(buf, file_table[fd].open_offset + file_table[fd].disk_offset, read_len);
  }
